...
export class BaseModel {
   ...
   */
  static async handleCollectionResponse(
  ...
  static parsePaginated<T>(response: {
    ...
    const returnedArray: Array<IModelData> = response.result
    const collection: BaseModel[] = returnedArray.map((modelData) => {
      return new this(modelData)
    })
    return {
      result: collection,

...
export function ApiEndpoint(
  ...
  urlOptions: IApiEndpointDecoratorOptions
) {
  return function (
    target: BaseModel | typeof BaseModel,
    propertyName: string,
    descriptor: PropertyDescriptor
  ): any {
    const originalFunction = descriptor!.value

    const requestFunction = async function (
      this: BaseModel | any,
      options: IRequestOptions = {}
    ): Promise<any> {
      options.caller = this
      options.url = endpointUrlFactory.make(
        urlOptions.url,
        options.urlParams,
        options.transformFinalUrl
      )
